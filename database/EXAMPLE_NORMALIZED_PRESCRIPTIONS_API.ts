// EXAMPLE: Prescriptions API with Normalization
// File: app/api/prescriptions/route.ts (NORMALIZED VERSION)

import { NextResponse } from 'next/server';
import { query, execute } from '@/lib/db';
import { getSession } from '@/lib/auth';

// GET all prescriptions - Using VIEW for normalized data
export async function GET(request: Request) {
  try {
    ...
    let sql = `SELECT * FROM V_PRESCRIPTIONS`;
    ...
    */
    ...
    const transformed = prescriptions.map((p: Record<string, unknown>) => ({
      ...
      PRES_PAT_NAME: p.PRES_PAT_NAME || p.pres_pat_name, // From JOIN/View
      PRES_DOC_NUMBER: p.PRES_DOC_NUMBER || p.pres_doc_number,
      PRES_DOC_NAME: p.PRES_DOC_NAME || p.pres_doc_name, // From JOIN/View
...
// POST - Create new prescription (NORMALIZED)
export async function POST(request: Request) {
  try {
    ...
    // NORMALIZED: Only store FK references, not names
    const sql = `
    ...
    await execute(sql, [
      prescriptionNumber,
      body.patientNumber,      // FK only, no patientName
      body.doctorNumber || null, // FK only, no doctorName
      body.medication,
      body.dosage || null,
      body.frequency || null,
      body.duration || null,
...
export async function PUT(request: Request) {
  try {
    ...
    // No need to update names - they come from JOIN
    const sql = `
